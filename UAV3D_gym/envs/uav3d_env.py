...
class UAV3DEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = [space.Discrete(len(self.uavs[0].avail_phi)) for _ in range(self.n_agents)]
        self,observation_space = [space.Box(-high, high, dtype=np.float32) for _ in range(self.n_agents)]
        self.seed()
    ...
    def step(self, action:[]):
        ...
        for _ in range(A_REPEAT):
            ...
            for i in range(self.n_agents):
                s, _, _, = self.uavs[i].step(action[i])
            ...
            # judge if uavs die (die condition including OOB)
